...
C      Update strain
      ...
      stran(i,4:6)=stateOld(i,11:13)+strainInc(i,4:6)
      stateNew(i,8:13)=stran(i,1:6)
        
C      Calculate the strain invariants
      I1=stran(i,1)+stran(i,2)+stran(i,3)
      I2=stran(i,1)*stran(i,2)+stran(i,2)*
     1    stran(i,3)+stran(i,1)*stran(i,3)-
     2    stran(i,4)**2-stran(i,5)**2-stran(i,6)**2
      I3=stran(i,1)*stran(i,2)*stran(i,3)
     1    -stran(i,1)*stran(i,5)**2-stran(i,2)
     2    *stran(i,6)**2-stran(i,3)*stran(i,4)**2
     3    +2*stran(i,4)*stran(i,5)*stran(i,6)
...
C      PF-CZM parameters for cornelissen softening law
